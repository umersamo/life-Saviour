...
import getLocation from './FetchLocation';
  ...
  let check = (e)=>{
    ...
    if(organ === "" || desc === ""){
      ...
      let id=Date.now()
      firebase.database().ref("lifesaviour/organ/"+id).set({
        id:id,
      ...
      <div>
        ...
        <Popper open={open} anchorEl={anchorRef.current} role={undefined} transition disablePortal style={{zIndex:1000}}>
          {({ TransitionProps, placement }) => (
            ...
            <h2>Donate Organ</h2>
            <br></br>
            <TextField style ={styles.TextField}  label="Enter Organ name" variant="outlined" onChange={(e)=>{setorgan(e.target.value)}}/>
            <TextField style ={styles.TextField}  label="Any description" variant="outlined" onChange={(e)=>{setdesc(e.target.value)}} />
